...
(function($, anim) {
  var methods = {
    init: function(options) {
      ...
      return this.each(function(i) {
        ...
        var inDuration =
          parseInt($(this).attr("data-in-duration")) || options.inDuration;
        var outDuration =
          parseInt($(this).attr("data-out-duration")) || options.outDuration;
        var responsiveThreshold =
          parseInt($(this).attr("data-responsive-threshold")) ||
          options.responsiveThreshold;
        var originalContentPadding =
          parseInt($(this).attr("data-content-padding")) ||
          options.contentPadding;
        var primaryColor =
          $(this).attr("data-primary-color") || options.primaryColor;
        var secondaryColor =
          $(this).attr("data-secondary-color") || options.secondaryColor;
        var fillScreen =
          !!$object.length &&
          ($(this).attr("data-fill-screen") || options.fillScreen);
        var dynamicRouting =
          $(this).attr("data-dynamic-routing") || options.dynamicRouting;
        var isHorizontal = $(this).hasClass("gallery-horizontal");
        var fullWidth,
          objectSizeAdjusted = false;
        ...
        var setOrigDimensions = function() {
          ...
          origObjectRect = $object.length
            ? $object[0].getBoundingClientRect()
            : { top: origHeaderRect.top, left: 0 };
          origScrollTop = $(window).scrollTop();
        };

        origin
          .off("click.galleryExpand")
          .on("click.galleryExpand", function(e) {
            ...
            var target = $(e.target);
            if (
              target.attr("data-stop-propagation") ||
            ...
            var recalculateVars = function() {
              ...
              objectSizeAdjusted =
                origObjectWidth > windowWidth / 2 || fillScreen;
              bodyScrolls =
              ...
              offsetLeft =
                offsetLeft === undefined
                  ? -placeholder[0].getBoundingClientRect().left
                  : offsetLeft + -placeholder[0].getBoundingClientRect().left;
              contentPadding = fullWidth ? 20 : originalContentPadding;
              contentOffsetTop = Math.round(
                origObjectHeight / 2 + contentPadding + navOffset
              );
              ...
              cardPadding = Math.max(
                (windowWidth - newCardWidth) / 2,
                contentPadding
              );
              ...
              if (objectSizeAdjusted) {
                if (fillScreen) {
                  ...
                  adjustedObjectHeight = Math.min(
                    windowHeight - navOffset,
                    imgHeight
                  );

                  if (fullWidth) {
                    contentOffsetTop = Math.round(
                      (windowHeight - navOffset) / 2
                    );
                  } else {
                    contentOffsetTop = Math.round(
                      contentPadding * 3 + navOffset
                    );
                  }
                  headerOffsetTop =
                    adjustedObjectHeight - contentOffsetTop + navOffset;
                  ...
                  contentOffsetTop =
                    Math.round(adjustedObjectHeight / 2) +
                    contentPadding +
                    navOffset;
            ...
            $("body").on(
              "scroll.disable-scroll mousewheel.disable-scroll touchmove.disable-scroll",
              function(e) {
                e.preventDefault();
                e.stopPropagation();
                return false;
            ...
            if ($object.length) {
              ...
              if (fillScreen) {
                transformString +=
                  origObjectRect.left + "px, " + contentPadding + "px, 0)";
              } else {
                $object.css({
            ...
            var curveWrapperTop =
              origObjectRect.top - navOffset - contentPadding;
            $curveWrapper.css({
            ...
            origHeaderOffsetTop = isHorizontal
              ? contentPadding - origHeaderHeight
              : contentPadding;
            $header.css({
            ...
            var backBtn = $(
              '<a class="waves-effect waves-light black btn">뒤로가기</a>'
            );
            ...
            if (typeof ColorThief !== "undefined" && $object.length) {
              ...
              try {
                sampledColorPalette = colorThief.getPalette(
                  origin.find("img")[0],
                  2
                );
                primaryColor =
                  primaryColor ||
                  "rgb(" +
                    sampledColorPalette[0][0] +
                    "," +
                    sampledColorPalette[0][1] +
                    "," +
                    sampledColorPalette[0][2] +
                    ")";
                secondaryColor =
                  secondaryColor ||
                  "rgb(" +
                    sampledColorPalette[1][0] +
                    "," +
                    sampledColorPalette[1][1] +
                    "," +
                    sampledColorPalette[1][2] +
                    ")";
                ...
                console.log(
                  "Cross Origin error. Falling back to defaultColor. Try using a locally hosted image",
                  e
                );
                ...
                if (tempCanvas.length) {
                  ...
                  if (
                    tempCanvas[0].getContext("2d").canvas.width === imgWidth &&
            ...
            objectCssObject["-webkit-transition"] =
              "-webkit-transform " + inDuration / 1000 + "s";
            ...
            if (objectSizeAdjusted) {
              ...
              if (fillScreen) {
                ...
                objectCssObject["-webkit-transition"] =
                  ...
                  "s";
                var scaleRatio =
                  adjustedObjectWidth < windowWidth
                    ? windowWidth / adjustedObjectWidth
                    : 1;
                if (adjustedObjectWidth < windowWidth) {
                  objectCssObject.transform =
                    "scale(" + windowWidth / adjustedObjectWidth + ")";
                }
              } else {
                objectCssObject.transform =
                  "scale(" + adjustedObjectWidth / origObjectWidth + ")";
              }
            }
            ...
            $curveWrapper.css({
              ...
              "-webkit-transition":
                "-webkit-transform " + inDuration / 1000 + "s " + bezierCurve,
              transform: ""
            });
            ...
            var transformHeader = function() {
              ...
              if (objectSizeAdjusted && !fillScreen) {
                offsetTop =
                  (headerOffsetTop - origObjectHeight) / cardHeightRatio;
              }

              $header.css({
                transition:
                  "transform " + inDuration / 1000 + "s " + bezierCurve,
                "-webkit-transition":
                  "-webkit-transform " + inDuration / 1000 + "s " + bezierCurve,
                transform:
                  "scale(" +
          ...
          setTimeout(function() {
            ...
            if (!cancelledTimeout) {
              ...
              origin.css({
                ...
                transition:
                  "transform " + outDuration / 1000 + "s " + bezierCurve,
                "-webkit-transition":
                  "-webkit-transform " + outDuration / 1000 + "s " + bezierCurve
              });
              ...
              $curveWrapper.css({
                ...
                "-webkit-transition":
                  "-webkit-transform " + outDuration / 1000 + "s"
              });
              ...
              if (objectSizeAdjusted && fillScreen) {
                ...
                objectCssObject.transform =
                  "translate3d(" +
                  (origObjectRect.left - cardPadding - contentPadding) +
                  "px, 0, 0)";
                objectCssObject.transition =
                  "transform " + outDuration / 1000 + "s " + bezierCurve;
                objectCssObject["-webkit-transition"] =
        ...
        if (dynamicRouting && urlObjectId === objectId) {
          var img = $object.children("img").first();
          if (img.length) {
            img
              .on("load", function() {
  ...
  $.fn.galleryExpand = function(methodOrOptions) {
    if (methods[methodOrOptions]) {
      ...
      $.error(
        "Method " + methodOrOptions + " does not exist on jQuery.galleryExpand"
      );
    }
  }; // Plugin end
})(jQuery, M.anime);
